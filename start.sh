...
# Check if this is first run (no backend/venv exists)
if [ ! -d "backend/venv" ]; then
    echo "⚠️  First time setup detected!"
    echo "Running initialization script first..."
    ./init.sh
    echo ""
fi
...
# Only install dependencies if requirements.txt is newer than install timestamp
REQUIREMENTS_FILE="requirements.txt"
INSTALL_MARKER=".pip_install_complete"

if [ "$REQUIREMENTS_FILE" -nt "$INSTALL_MARKER" ] || [ ! -f "$INSTALL_MARKER" ]; then
    echo "📦 Installing/updating dependencies..."
    pip install -r requirements.txt && touch "$INSTALL_MARKER"
else
