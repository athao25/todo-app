...
# Todo App Initialization Script
# This script initializes both frontend and backend components
...
main() {
    ...
    echo "1. Update backend/.env.local with your database credentials (if needed)"
    echo "2. Start both servers: ./start.sh"
    echo "3. Open http://localhost:3000 in your browser"
    echo ""
    print_status "Alternative startup methods:"
    echo "• Manual: Start each server separately"
    echo "  - Backend: cd backend && source venv/bin/activate && python app.py"
    echo "  - Frontend: cd frontend && npm run dev"
    echo "• Docker: docker-compose up -d"
    echo "• NPM: npm run start"
    echo ""
